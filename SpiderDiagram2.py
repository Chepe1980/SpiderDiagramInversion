...
def get_color_for_score(score):
    """Get color based on score (red for low, green for high)"""
    if score >= 8:
        return 'green'
    elif score >= 6:
        return 'yellowgreen'
...
def create_spider_chart(scores, weights):
    """Create the 2D spider/radar chart with colormap"""
    categories = list(scores.keys())
    score_values = list(scores.values())
    
    # Create colors based on scores
    colors = [get_color_for_score(score) for score in score_values]
    colors.append(colors[0])  # Close the circle
    ...
    # Add project scores with gradient coloring
    fig.add_trace(go.Scatterpolar(
        ...
        name='Project Score',
        hovertemplate='<b>%{theta}</b><br>Score: %{r}/10<br>Quality: %{marker.color}<extra></extra>'
    ))
    
    # Add individual points with color coding
    for i, (category, score) in enumerate(zip(categories, score_values)):
        fig.add_trace(go.Scatterpolar(
            r=[score],
            theta=[category],
            mode='markers',
...
def create_3d_spider_chart(scores, weights):
    ...
    for i, (score, height) in enumerate(zip(score_values + [score_values[0]], height_values + [height_values[0]])):
        ...
        colors.append(score)  # Use score for colorscale
    
    fig = go.Figure()
    
    # Add the main surface with colorscale
    ...
    # Add markers at each vertex with color coding
    fig.add_trace(go.Scatter3d(
        x=x, y=y, z=z,
        mode='markers+text',
        marker=dict(
            size=8,
            color=score_values + [score_values[0]],
            colorscale='RdYlGn',
            cmin=0,
            cmax=10,
            colorbar=dict(
...
def create_3d_bars_chart(scores, weights):
    ...
    # Colors based on score using the same colormap
    colors = score_values
    
    fig = go.Figure()
    
    # Create 3D bars
    for i, (category, score, height, color_val) in enumerate(zip(categories, score_values, height_values, colors)):
        fig.add_trace(go.Mesh3d(
        ...
        weight_color = 'blue' if height == 1 else 'orange' if height == 2 else 'red'
        fig.add_trace(go.Mesh3d(
...
def main():
    ...
    with tab1:
        ...
        with col1:
            st.subheader("2D QC Spider Diagram with Colormap")
            st.markdown("""
            **Color Guide:**
            - 🟥 **Red**: Poor (0-4) - Needs immediate attention
            - 🟨 **Orange**: Fair (4-6) - Requires improvement
            - 🟩 **Green**: Good (6-10) - Acceptable to excellent
            """)
    ...
    with tab2:
        ...
        **Visualization Guide:**
        ...
        - **Color** indicates quality (Red=Poor, Yellow=Medium, Green=Good)
    ...
    with tab3:
        ...
        **Visualization Guide:**
        ...
        - **Base Color** indicates quality (Red=Poor, Green=Good)
        """)
